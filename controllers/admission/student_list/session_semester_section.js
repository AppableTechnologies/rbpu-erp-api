const { Program, Session, Semester, Section, ProgramSemesterSection, ProgramSemester } = require("../../../models");
...
//above is Earlier code of getSemestersViaProgramId
...
const getSemestersViaProgramId = async (req, res) => {
  ...
  try {
    ...
    const programSemesters = await ProgramSemester.findAll({
      where: { program_id }, // ✅ filter properly
      include: [
        {
          model: Semester,
          attributes: ["id", "title"],
    ...
    if (!programSemesters || programSemesters.length === 0) {
      return res.status(404).json({ message: "Semester not found." });
    }

    // Extract semesters only
    const semesters = programSemesters.map((ps) => ps.Semester);
